...
params:
  ...
  startdate: !r (Sys.Date() -2) %m+% months(-1)
  timestep: "weekly"
...
add_anim_theme <- function(bbox = params$bbox, subtitle_text_col = "snow"){
    ...
    # set map extent
    coord_sf(xlim = c(make360(params$bbox$xmin+1), make360(params$bbox$xmax)), ylim = c(params$bbox$ymin, params$bbox$ymax), expand = FALSE, crs = st_crs(4326)),
...
add_bathy <- function(data = bathy_df, depths = c(-200), cols = c("azure3")){
    list(
...
anim_caption = str_c("\n Raw tracking data from ARGOS averaged to 1 " , params$timestep , " location per turtle (circles)\n\n", 
                         ...
                         "Thermal Corridor region (box). Ship release location (X)\n",
                          caption_iso, "Continental depths of 200m (light gray lines)\n",
    ...
    lapply(seq(1,n), function(i) {
        ...
        # plot up
            ...
            # add bathy
                {
                    if (params$bathy) {
                        add_bathy(data = bathy_df, depths = c(-200), )
                    }
                } +
        ...
        add_anim_theme(bbox = params$bbox, subtitle_text_col = "snow")
...
if(save_figs){
    lapply(seq(1:length(wk_names)), function(i) { ggsave(tracks_plot_list[[i]], 
                                                         file = str_c('../anim_figs/', i, "_", dates[i], "_", params$eov, "_tracks.png"), 
                                                         width=14, 
                                                         # scale = 2,
...
image_write(image = plot_animated,
            path = str_c('../anim_figs/', "stretch_2023_",params$timestep,"_animation_", params$enddate, "_", params$eov, ".gif"))
