...
if(params$release_yr == "23"){
    files = list.files('~/Downloads/batch/', pattern = "All.csv",recursive=T, full.names=T) # temp location while tags still running - change dir later
} else if(params$release_yr == "24"){
    files = list.files('~/Downloads/batch_cohort2/', pattern = "All.csv",recursive=T, full.names=T) # temp location while tags still running - change dir later
}
```
...
if(params$release_yr == "23"){
    ...
    raw_data <- raw_data %>%
        mutate(date = as.POSIXct(date, format= "%m/%d/%Y %H:%M:%S", tz="UTC")) %>%
        filter(lat > 0 & lat < 60) %>%
        filter(date >= '2024-07-08 04:30:00') %>%
        filter(loc_quality != 0)  
}
...
```{r 01-rename dfs}
raw_df <- raw_data_cohort_w_names
daily_avg_data <- daily_avg_data_cohort
...
deploy_date <- as.Date("2023-07-10")
    ...
    deploy_date <- as.Date("2024-07-07")
}
...
if(length(fdates) != length(ncIn)){
    ...
    names(ras) <- fdates  # double check this works.
    nc_dates <- fdates
...
if(params$eov == 'sst'){
    ...
    # revised oct 2024 to deal with extreme vals / cbar inconsistencies
    
    cbar_breaks <- seq(-4,4.5,0.5)
    cbar_limits <- c(-4,4.5)
    cbar_int <- 0.5
    cbar_title <- 'SSTa (°C)\n'
    
    cpal <- rev(c("#48090B", "#540b0e", rev(brewer.pal(9, "YlOrRd")),"white", brewer.pal(9, "Blues"), "#06224C", "#031126"))
    
    zCuts <- seq(-5.5,6,0.5)
   
    cbar_breaks <- c('≥ 4', cbar_breaks[2:16], '≤ -4')
...
if(params$release_yr == "23"){
    ...
    release_loc = data.frame(lat=39.55778, lon=211.5045)
}
...
add_cclme <- function(data = cclme_df){
    list(
         ...
         geom_polygon(data = data, aes(x = make360(long), y = lat.x, group = group), fill = NA, color = 'gray85', alpha = 0.5) 
    )
}
...
add_release_location <- function(data = release_loc){
    list(
        geom_point(aes(x=make360(release_loc$lon), y=release_loc$lat),colour="gray20", size=1),
        geom_point(aes(x=make360(release_loc$lon), y=release_loc$lat),shape = 21,size = 2,colour = "gray20", stroke = 1)
    )
}

add_release_location_label <- function(data = release_loc, seg_x=2, seg_y=1, label = "Release\nLocation"){
    list(
        annotate(geom = "text", x = data$lon-seg_x, y = data$lat-seg_y, label = label, hjust = "right")
    )
}

add_release_location_segment <- function(data = release_loc, seg_x=2, seg_y=1){
    list(
        annotate(
        geom = "segment", x = data$lon, y = data$lat, xend = data$lon-seg_x, yend = data$lat-seg_y)
    )
}

annotate_text <- function(text_x = make360(-122), text_y = 20, label = " Release Location", text_col = "white", text_size = 4){
    list(
        geom_text(size=4, colour=text_col,
          aes(label = label, x = text_x, y = text_y), vjust = 0, hjust = 0)
          # annotate("pointrange", 
          #  x = text_x-4,
          #  y = text_y, ymin = text_y, ymax = text_y,
          #  label = label, col = text_col, size = pt_size),        
          # annotate("text", 
          #  x = text_x,
          #  y = text_y, 
          #  label = label, col = text_col, size = text_size)
    )
}
...
# add_tcms_box_legend <- function(text_x = make360(-122), text_y = 19, label = " Thermal Corridor Study Area", text_col = "white", text_size = 4){
#     list(
#         # geom_text(size=4, colour=text_col, 
#         #   aes(label = label, x = text_x, y = text_y), vjust = 0, hjust = 0)
#           annotate("text", 
...
# set specific color pal length
n_cols <- length(unique(turtles_df$id))


idColors <-
  setNames( c(rainbow(n_cols))
...
add_sst_cpal <- function(labels = cbar_breaks, cbar_title){
    list(
        ...
        # scale_fill_manual(values = cpal[3:length(cpal)], name = cbar_title,
        ...
        scale_fill_gradientn(colours = unname(cpal[3:length(cpal)]),
                                super = metR::ScaleDiscretised, guide = guide_legend(label.vjust=+1.2, barwidth = 1, frame.colour = "black", ticks.colour = "black", ncol =1, reverse=T))
    )
}
...
add_ssta_cpal <- function(labels = cbar_breaks, cbar_title){
    list(
        scale_fill_manual(#values = cpal[1:length(cpal)], 
          values = cpal[4:length(cpal)], 
          name = cbar_title, 
          labels = cbar_breaks, drop = F, na.translate = F, 
          guide = guide_legend(label.vjust=+1.2, barwidth = 1, frame.colour = "black", ticks.colour = "black", ncol =1, reverse=F)) 
    )
...
if(params$eov == 'sst'){
    ...
    subtitle_text_col <- 'gray8'
        caption_iso <- '' #'The 0.2 mg/m^3 isopleth (black line) represents the approximate TZCF position in the eastern North Pacific. '
        eov_source <- 'Chlorophyll-a, Aqua MODIS, Weekly Composite 0.025° Chl Concentration'
...
```{r}
# placeholder numbers for now -- need to dynamically update
if(params$subset_tcms){
    n_indivs = 7
} else {
    if(params$release_yr == "23"){
        n_indivs = length(unique(turtles_df_anim$id))
    } else if(params$release_yr == "24"){
        n_indivs = length(unique(turtles_df_anim$id))
    }
...
anim_title <- glue::glue("STRETCH ", str_to_title(params$timestep), " turtle movements (n={n_indivs}) with ", title_eov)
...
anim_caption = str_c("\n Raw tracking data from ARGOS averaged to 1 " , params$timestep , " location per turtle (color lines)\n\n", 
                         "California Current Large Marine Ecosystem (CCLME) shaded in gray\n", 
                         # "Continental depths of 200m (gray lines). Ship release location (white circles)\n",
                     "Continental depths of 200m (gray lines)\n",
...
if(identical(unique(turtles_df_anim$date),unique(eov_df_anim$date))){
    ...
    if (params$timestep == 'daily'){
        ...
        if(params$release_yr == '23'){
            ...
            dates <- seq(as.Date('2023-12-03'), as.Date(params$enddate), by = "1 day") # special for alberto
            
        } else if (params$release_yr == '24'){ 
            
            # dates <- seq(as.Date('2023-12-03'), as.Date(params$enddate), by = "1 day") # special for alberto
            dates <- seq(as.Date(min(turtles_df_anim$date)), as.Date(max(turtles_df_anim$date)), by = "1 day") # assumes start of week
            
        }
        
    } else if (params$timestep == 'weekly' & !params$subset_tcms){
    ...
    } else if (params$timestep == "weekly"  & params$subset_tcms | params$eov == "MBchla8day_LonPM180"){
...
tracks_plot_list <- 
    lapply(seq(1,n), function(i) {
        ...
        # set spatial extent for calculating n distinct indivs in frame
        if(params$subset_tcms){
            n_indivs <- turtles_frame %>% filter(lat >= 25, lat <= 37, lon >= -138 & lon <= -110) %>%
            summarise(n = n_distinct(id))
        } else {
        n_indivs <- turtles_frame  %>%
            summarise(n = n_distinct(id))
        }
        ...
        # plot up
            ...
            # add release location and text
            add_release_location(release_loc) +
            add_release_location_label(data = release_loc, seg_x = 2, seg_y = 1, label = "Release\nLocation") +
            add_release_location_segment(data = release_loc, seg_x = 2, seg_y = 1) +
...
tracks_plot_list <-
    lapply(seq(1,n), function(i) {
        tracks_plot_list[[i]] <- tracks_plot_list[[i]] +
            ...
            {
                if (params$eov =='sst'){
                    guides(fill = guide_legend(title = cbar_title, ncol=1, reverse=T, #barheight = panel_height, 
                                               limits = cbar_breaks,
                                               ticks = TRUE))
                } else if(params$eov == 'ssta')  {
                    guides(fill = guide_legend(title = cbar_title, ncol=1, reverse=T, #barheight = panel_height, 
...
## Add N Compass | Scalebar - pick up later
```{r}
# library(ggspatial)
# 
# tracks_plot_list <-
...
#         tracks_plot_list[[i]] <- tracks_plot_list[[i]] +
# 
#         ggsn::scalebar(x.min = make360(-168), x.max = make360(-110),
#                        # y.min = 16.5, y.max = 50,
#                        y.min = 26.5, y.max = 50,
#                        dist = 300, transform = TRUE,
#                        height = 0.01,
#                        st.dist = 0.02, st.bottom = TRUE, st.size = 2,
#                        
#                        location = "bottomleft",
#                        dist_unit = "km",
#                        model = 'WGS84',
...
# 
# 
#     ggsn::north2(tracks_plot_list[[i]],  x = 0.235, y=0.15, scale = 0.05, symbol = 1) +
# 
#     ggspatial::annotation_north_arrow(location = "bl", which_north = "true",
#                            pad_x = unit(0.68, "in"), pad_y = unit(0.5, "in"),
...
image_write(image = plot_animated,
            # path = str_c(dir, "stretch_2023_",params$timestep,"_animation_", params$enddate, "_", params$eov,"_", p_extent ,"fps_1.gif"))
            path = str_c(dir, "stretch_2024_",params$timestep,"_animation_", params$enddate, "_", params$eov,"_", p_extent ,"fps_2.gif"))
